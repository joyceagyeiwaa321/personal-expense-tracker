...
namespace FinancyApplication
{
    public class Data
    {
        ...
        {
            string query = "UPDATE user SET IsActive = " + (isActive ? 1 : 0) + " WHERE UserID = " + userId;
            this.ExecuteSimple(query);
        }
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        t.GroupID = reader["GroupID"] == DBNull.Value ? 0 : Convert.ToInt32(reader["GroupID"]);
                        ...
                        t.Status = reader["Status"] != DBNull.Value ? reader["Status"].ToString() : "Pending";
                        transactions.Add(t);
                    }
                }
                catch (Exception ex)
        ...
        public List<RecurringTransaction> GetRecurringByUser(int userId)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                string query = "SELECT rt.* FROM recurring_transaction rt " +
                               ...
                               "WHERE a.UserID = @userId ORDER BY rt.NextRunDate ASC";
                MySqlCommand cmd = new MySqlCommand(query, connection);
                cmd.Parameters.AddWithValue("@userId", userId);
                try
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        rt.RecurringId = Convert.ToInt32(reader["RecurringID"]);
                        rt.AccountId = Convert.ToInt32(reader["AccountID"]);
                        rt.CategoryId = Convert.ToInt32(reader["CategoryID"]);
                        rt.Type = reader["Type"].ToString();
                        rt.Amount = Convert.ToDecimal(reader["Amount"]);
                        rt.Frequency = reader["Frequency"].ToString();
                        rt.StartDate = Convert.ToDateTime(reader["StartDate"]);
                        rt.NextRunDate = Convert.ToDateTime(reader["NextRunDate"]);
                        rt.IsActive = Convert.ToInt32(reader["IsActive"]) == 1;
                        list.Add(rt);
                    }
                }
                catch (Exception ex)
        ...
        public int InsertTransaction(Transaction t)
        {
            ...
            string gIDValue = t.GroupID > 0 ? t.GroupID.ToString() : "NULL";
        ...
        public int InsertCategory(Category cat)
        {
            string query = "INSERT INTO category(UserID, Name, Type, IsDefault) VALUES(" +
                           cat.UserID + ", '" + cat.Name + "', '" + cat.Type + "', " + (cat.IsDefault ? 1 : 0) + ");";
            return this.Insert(query);
        }
        ...
        {
            string query = "UPDATE user SET IsVerified = " + (isVerified ? 1 : 0) + " WHERE UserID = " + userId;
            this.ExecuteSimple(query);
        }
        ...
        public int InsertRecurringTransaction(RecurringTransaction rt)
        {
            ...
            string query = "INSERT INTO recurring_transaction (AccountID, CategoryID, Type, Amount, Frequency, StartDate, NextRunDate, IsActive) " +
                           "VALUES (" + rt.AccountId + ", " + rt.CategoryId + ", '" + rt.Type + "', " + amount + ", '" +
                           rt.Frequency + "', '" + rt.StartDate.ToString("yyyy-MM-dd HH:mm:ss") + "', '" +
                           rt.NextRunDate.ToString("yyyy-MM-dd HH:mm:ss") + "', " + (rt.IsActive ? 1 : 0) + ");";
            return this.Insert(query);
        }
        ...
        public void UpdateRecurringTransaction(RecurringTransaction rt)
        {
            ...
            string query = "UPDATE recurring_transaction SET " +
                           "CategoryID = " + rt.CategoryId + ", Type = '" + rt.Type + "', Amount = " + amount + ", " +
                           "Frequency = '" + rt.Frequency + "', StartDate = '" + rt.StartDate.ToString("yyyy-MM-dd HH:mm:ss") + "', " +
                           "NextRunDate = '" + rt.NextRunDate.ToString("yyyy-MM-dd HH:mm:ss") + "', IsActive = " + (rt.IsActive ? 1 : 0) +
        ...
        public int InsertReceipt(Receipt receipt)
        {
            string query = "INSERT INTO receipt(TransactionID, FilePath, FileType, UploadedAt) VALUES(" +
                           receipt.TransactionID + ", '" + receipt.FilePath + "', '" + receipt.FileType + "', '" +
        ...
        {
            string query = "UPDATE `group` SET Name = '" + group.Name + "' WHERE GroupID = " + group.GroupID;
            this.ExecuteSimple(query);
        }
        ...
        {
            string query = "DELETE FROM group_member WHERE GroupID = " + groupId + " AND UserID = " + userId;
            this.ExecuteSimple(query);
        }
                ...
                try
                {
                    ...
                    if (reader.Read())
                    {
                        return new User
                        {
                            ...
                            Role = reader["Role"].ToString() == "Admin" ? UserRole.Admin : UserRole.User,
        ...
        public List<Category> GetCategoriesByUser(int userId)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                string query = "SELECT * FROM category WHERE UserID = @userId OR IsDefault = 1 OR UserID IS NULL";
                MySqlCommand cmd = new MySqlCommand(query, connection);
                cmd.Parameters.AddWithValue("@userId", userId);
                try
                {
