...
namespace FinancyApplication
{
	public class OpenAiService
	{
		...
		public static async Task<Dictionary<string, string>> ScanReceipt(string imagePath, string apiKey)
		{
			...
			string prompt =
				"Look at this receipt and extract the following. " +
				"Reply ONLY with a JSON object with these exact keys: Amount, Description, Date. " +
				"Amount = total amount as a number only (e.g. 12.50). " +
				"Description = merchant or store name. " +
				"Date = in yyyy-MM-dd format. " +
				"If you cannot find a value use an empty string. " +
				"Example: {\"Amount\":\"25.99\",\"Description\":\"Shell Petrol\",\"Date\":\"2026-05-29\"}";
			...
			if (!response.IsSuccessStatusCode)
				throw new Exception("Vision API error: " + responseText);
			...
			if (reply.Contains("{"))
			{
				int start = reply.IndexOf('{');
				int end = reply.LastIndexOf('}');
				if (end > start) reply = reply.Substring(start, end - start + 1);
			}
